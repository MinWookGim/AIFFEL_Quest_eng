...
------------------------------------------------------------------------------------
  ...
  4컷 전부일치             ~0.000   0.021    0.083

  주의: 생성 평가에는 **Gram 을 쓴다.** CLIP 은 천장이 0.25 라 잴 수 있는 폭이 너무 좁다.

  Gram 기준 style_precision 분포:  10%=0.000  25%=0.250  중앙=0.500  75%=0.750
    - 0.25(4컷 중 1컷) 이상: 진짜 그림 84.7% 통과 / 무작위도 37.6% 통과  -> 너무 헐겁다
    - 0.50(4컷 중 2컷) 이상: 진짜 그림 55.5% 통과 / 무작위는  6.3% 통과  -> **여기가 통과선**
      (무작위 확률은 이항분포 B(4, 1/9) 로 계산)

  컷 간 절대 코사인은 판별에 실패했다 (같은 그림체 0.668 vs 다른 그림체 0.653).
...
QUALITY_THRESHOLDS = {
    ...
    "style_precision_min": 0.50,    # 진짜 그림 중앙값. 무작위(0.111)가 통과할 확률 6.3%
    ...
    # ── 복붙 방지 (상한) ─────────────────────────
    "edition_cosine_max": 0.95,     # 같은 그림 4장이면 1.000. 정상 범위는 0.584~0.770
